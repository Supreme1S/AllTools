#!/usr/bin/env python3
"""Full Russian prose for all 55 DEFI_BATCH_1 catalog entries."""
...
CONTENT: dict[str, dict] = {
    ...
    "9inch": {
        ...
        "warnings": ["PulseChain и fork-DEX — экзотика с повышенным риском smart contract и выхода из ликвидности."],
    },
    "aave": {
...
# Import remaining from part modules (will be fully rewritten)
...
def main() -> None:
    slugs = json.loads((ROOT / "defi-batch-1.json").read_text(encoding="utf-8"))
    ...
    for slug, e in ordered_content.items():
        if len(e.get("facts", [])) != 3:
            raise SystemExit(f"{slug}: expected 3 facts, got {len(e.get('facts', []))}")
    OUT_JSON.write_text(json.dumps(ordered_content, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {len(ordered_content)} entries to {OUT_JSON}")
